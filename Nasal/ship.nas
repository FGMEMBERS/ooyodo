...
var sub = "sim/SDM/submersible";
var depth = "position/depth-m";
var maxdepth = "sim/SDM/max-depth-m";
var valve = "sim/SDM/controls/valve-norm";
var blow = "sim/SDM/controls/blow-norm";
var diving = "sim/SDM/dive-factor";
var rising = "sim/SDM/rise-factor";
var fwdplane ="sim/SDM/controls/fwd-diveplane-norm";
var rearplane ="sim/SDM/controls/rear-diveplane-norm";
var diveplanefactor = "sim/SDM/diveplane-effectivity-factor";

var ballast0 = "sim/SDM/ballast[0]/ballast-norm";
...
var update_orientation = func {
    ...
    var planes= (getprop (fwdplane)- getprop(rearplane))* (getprop (speed)/getprop (max_speed));
    #print (planes);

    setprop (pitch, pitchup + planes*getprop (diveplanefactor));
}
...
var check_ground = func {
    ...
    var altz= getprop(depth);
#   print ("2depth=",altz);
    var alt1 = (alt0+10.2+altz)*FT2M;
...
#   print (alt0);
    ...
    if (getprop (sub)) {
        altz = getprop (ballast0)*getprop (maxdepth);
    #   print ("depth: ",alt0);
    }
    setprop (depth, altz);
    setprop ("position/altitude-ft",alt0-altz);
...
var update_dive = func () {
    var valveopen = getprop (valve);
    var blowing = getprop (blow);
    var ballast = getprop (ballast0);
    if (ballast < 0 ) {
        ballast =0;
    }
    if ( valveopen > 0) {
        interpolate (ballast0, ballast + getprop(diving)*valveopen , looptime);
        #print ("Diving");
    }
    if (blowing > 0) {
        interpolate (ballast0, ballast -getprop(rising)*blowing , looptime);
        #print ("Surfacing");
